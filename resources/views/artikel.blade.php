...
<style>
    ...
    .artikel-doc-link svg { width: 12px; height: 12px; stroke: currentColor; }
            ...
            <div class="artikel-featured">
                <div class="artikel-card featured-main">
                    ...
                    <div class="artikel-body">
                        ...
                        @if($featured->documents->count() > 0)
                            @foreach($featured->documents->take(2) as $doc)
                            <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="artikel-doc-link">
                                <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                                {{ strtoupper(pathinfo($doc->file_path, PATHINFO_EXTENSION)) }}
                            </a>
                            @endforeach
                            @if($featured->documents->count() > 2)
                            <span style="font-size:11px;color:var(--text-muted);">+{{ $featured->documents->count() - 2 }} dokumen lainnya</span>
                            @endif
                        @endif
                ...
                <div class="artikel-card">
                    ...
                    <div class="artikel-body">
                        ...
                        @if($second->documents->count() > 0)
                            @foreach($second->documents->take(2) as $doc)
                            <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="artikel-doc-link">
                                <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                                {{ strtoupper(pathinfo($doc->file_path, PATHINFO_EXTENSION)) }}
                            </a>
                            @endforeach
                            @if($second->documents->count() > 2)
                            <span style="font-size:11px;color:var(--text-muted);">+{{ $second->documents->count() - 2 }} lainnya</span>
                            @endif
                        @endif
                    </div>
                ...
                <div class="artikel-card">
                    ...
                    <div class="artikel-body">
                        ...
                        @if($artikel->documents->count() > 0)
                            @foreach($artikel->documents->take(2) as $doc)
                            <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="artikel-doc-link">
                                <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                                {{ strtoupper(pathinfo($doc->file_path, PATHINFO_EXTENSION)) }}
                            </a>
                            @endforeach
                            @if($artikel->documents->count() > 2)
                            <span style="font-size:11px;color:var(--text-muted);">+{{ $artikel->documents->count() - 2 }} lainnya</span>
                            @endif
                        @endif
                    </div>
                    <div class="artikel-footer">
